...
%% Last-Updated: 2020-01-20T13:49:25+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\item \label{item:step_calculate_probabilities} Use the
  ...
  answer of interest, conditional on the data observed and on our pre-data
  information:
  \begin{equation}
...
The expression of the data in terms of $\tu{\yNm_{j}}$ requires some more
thinking. We're still focusing on a specific input region (say, thalamus)
and a specific target class (say, parvalbumin). Our data consists, for a
number of mice, of the total number of target neurons marked by the virus,
and the total number of input neurons that have been infected by the target
ones, and thus also marked by the virus. Let's use a circumflex to indicate
observed quantities. The observed mice are indexed with $\ymh$, the marked
target neurons with $\yjh$, and the marked input neurons with $\yih$. The
total numbers of marked target and input neurons in mouse $\ymh$ are
$\yJhm$ and $\yIhm$.
...
Because of the way the virus infection works, our data tell us the
following; refer to the grid of \fig~\ref{fig:datamouse}:
\begin{itemize}
\item We don't know anything about the projections of marked and unmarked
  input neurons to the unmarked target neurons. This is represented by the
  yellow-bounded region on the right with the large yellow question mark.
\item We know that the unmarked input neurons \emph{cannot} project to the
  marked target neurons -- because otherwise they would have been infected
  and would be marked instead. Thus the lower-right, green-bounded part of the grid
  has no black entries.
\item We know that each marked input neuron must project to \emph{at least
    one} marked target neuron -- otherwise it wouldn't have been infected
  and marked. This means that \emph{each row} in the upper-right, red-bounded part
  of the grid in \fig\emph{must have at least one black entry} -- but we don't
  know how many black entries and where.
\end{itemize}
