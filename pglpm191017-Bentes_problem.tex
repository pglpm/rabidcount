...
%% Last-Updated: 2020-01-18T23:58:03+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
region, labelling them $i=1,2,\dotsc, I_{m}$, and all neurons constituting
the target class, labelling them $j=1,2,\dotsc, J_{m}$. The total numbers
of these neurons can be different from mouse to mouse, hence the index
\enquote{$m$} in $I_{m}$ and $J_{m}$.
...
target neuron $j$, but not on their individual identities. Let $N_{j}$
denote this number. In \fig~\ref{fig:cmij} it's the total number of black
squares in column $j$.

If we could experimentally count the projections and find $N_{j}$ for all
$J_{m}$ target neurons of mouse $m$,  we could then report several
statistics; for example:
\begin{itemize}[wide]
\item the average number of projections to the target neurons in that
  mouse: $\yNa \defd \frac{1}{J_{m}}\sum_{j}N_{j}$;
\item the variance from that average number,
  $\frac{1}{J_{m}}\sum_{j}\bigl(N_{j}-\yNa\bigr)^{2}$, or any other measure
  of variation;
\item the full histogram $\yH\defd(H_{0},H_{1}, H_{2},\dotsc)$ of the
  number of projections for that mouse: $H_{0}$ neurons receive no
  projection, that is, we have $N_{j}=0$ for $H_{0}$ different $j$s;
  $H_{1}$ neurons receive only one projection, that is, $N_{j}=1$ for
  $H_{1}$ different $j$s; $H_{2}$ receive two projections; and so on. From
  such histogram we can calculate the mean and variance above; for example
  $\yNa = \frac{1}{J_{m}}\sum_{n}n\,H_{n}$.
\end{itemize}

If we had these statistics and histogram $\yHm$ for each mouse $m$, we
could then ask about the average histogram of projections for \enquote{a
  typical mouse}, $\yHa \defd \frac{1}{M}\sum_{m} \yHm$, where $M$ is the
total number of mice; and also about the variance from such an average
histogram. These histograms and their average would be the answers to the
\enquote{individual} and \enquote{collective} questions we asked in
\sect~\ref{sec:synopsis}.

So, if we can give a probability distribution for the values $N_{j}$ for
each mouse,
\begin{equation}
  \label{eq:guess_prob_N}
  \pf(N_{1}, N_{2}, \dotsc \| \text{data}, \yI)
\end{equation}
