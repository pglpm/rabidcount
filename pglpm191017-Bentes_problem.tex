...
%% Last-Updated: 2020-05-25T14:38:04+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\propertitle}{Bayesian methods\\for inferring neuronal connectivity [draft]%\\{\large ***}%
}% title uses LARGE; set Large for smaller
\newcommand*{\pdftitle}{Bayesian methods for inferring neuronal connectivity [draft]}
\newcommand*{\headtitle}{Inferring connectivity}
\newcommand*{\pdfauthor}{P.G.L.  Porta Mana, B. Jacobsen, M. Witter}
\newcommand*{\headauthor}{Porta Mana, Jacobsen, Witter}
...
\hspace*{\stretch{1}}%
\parbox{0.33\linewidth}%\makebox[0pt][c]%
{\protect\centering M. Witter\\%
\footnotesize\epost{\email{menno.witter}{ntnu.no}}}%
\hspace*{\stretch{1}}%
...
\begin{abstract}\labelsep 0pt%
  \noindent The problem of inferring connectivity by means of
...
We can finally combine this with the probability~\eqref{eq:kb_given_stats}
for $\kb$, simplifying the combinatorial terms. We must also consider that
there are $\binom{\yIm}{\yIhm}$ ways in which the $\yIhm$ input cells could
have been selected out of $\yIm$, for each mouse. Considering all observed
mice $m$ we obtain
\begin{multline}
  \label{eq:prob_data_given_stats}
  \pf( \text{\small data} \|\tu{\yNm_{t}},  \tu{\yFm}, \yS, \yI) ={}\\
  \prod_{m}
  \binom{\yIm}{\yIhm}\,\sum_{r=0}^{\yIhm}(-1)^{r}\binom{\yIhm}{r}
...
\begin{multline}
  ...
  \pf( \text{\small data} \| \tu{\yFm}, \yS, \yI) ={}\\
  \prod_{m} \binom{\yIm}{\yIhm}\,
  \sum_{\yNm_{1}} \dotsi  \sum_{\yNm_{\yJhm}}
\prod_{t=1}^{\yJhm}
...
\binom{\yIhm-r}{N}  \bigg/ \binom{\yIm}{N}
\right]^{\yJhm}\\
\text{with } \text{\small data}=\set{\yIhm,\yJhm,\yIm}.
\end{empheq}
Note that the terms $\binom{\yIm}{\yIhm}$ can be included in the
proportionality constant.

\bigskip

As an initial exploration we can focus on one mouse and one input region.
The probability above simplifies to
\newcommand*{\cF}{\textcolor{mypurpleblue}{F}}
\newcommand*{\cI}{\textcolor{myred}{I}}
\newcommand*{\cT}{\textcolor{myred}{T}}
\newcommand*{\cS}{\textcolor{mygreen}{S}}
\begin{empheq}[box=\widefbox]{equation}
  \label{eq:final_form_answers_data_1mouse}
  \pf(\textcolor{mypurpleblue}{\yF}  \| \textcolor{myred}{\text{\small data}}, \textcolor{mygreen}{\varIota})
  \propto
  \sum_{r=0}^{\cI}(-1)^{\cI-r}\; \binom{\cI}{r}\;
  \left[
  \sum_{n=0}^{\cI}
\frac{\dbinom{r}{n}}{\dbinom{\cS}{n}}
\;  \textcolor{mypurpleblue}{F_{n}}
\right]^{\cT}
\times
\pf(\textcolor{mypurpleblue}{\yF}  \|  \textcolor{mygreen}{\varIota})
\end{empheq}

The first factor in the probability distribution above can be factorized.
First divide the statistics $\yF$ into two groups: $(F_{0}, \dotsc, F_{I})$
for inputs at most $I$, and $(F_{I+1}, \dotsc, F_{S})$ above $I$. The total
frequency for each group is
\begin{equation}
  \label{eq:groups_F}
  \yFi \defd F_{0} + \dotsb + F_{I}, \qquad
  \yFr \defd F_{I+1} +  \dotsb + F_{S}.
\end{equation}
Then consider the \emph{conditional frequencies} of a target cell's having
$0,\dotsc,I$ inputs, conditional on it having at most $I$ inputs:
\begin{equation}
  \label{eq:cond_freqs}
  f_{n} \defd \frac{F_{n}}{\yFi},\qquad
  \text{with } \sum_{n=0}^{I} f_{n} =1.
\end{equation}

From the distribution~\eqref{eq:final_form_answers_data_1mouse} we have
\begin{multline}
  \label{eq:factor_F_distr}
    \pf(\yf, \yFi, \yFr  \| \text{\small data}, \varIota)
  \propto{}\\
  \sum_{r=0}^{I}(-1)^{I-r}\; \binom{I}{r}\;
  \left[
  \sum_{n=0}^{I}
\frac{\dbinom{r}{n}}{\dbinom{S}{n}}
\;  f_{n}
\right]^{T}
\times \yFi^{T}
\times     \pf(\yf, \yFi, \yFr  \| \varIota) \;.
\end{multline}
Note that the probability for $\yFr$ does \emph{not} depend on the data.
